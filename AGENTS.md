...
开始酒馆前端联调时, Codex 应直接启动两个桌面上真实可见的 PowerShell 窗口, 并让它们长期运行:

1. `pnpm watch`
   - 负责持续编译代码, 并通过端口 `6621` 向酒馆网页推送实时更新事件.
2. `pnpm dlx http-server . -p 5500 --cors -c-1`
   - 负责通过端口 `5500` 提供 `dist` 中的编译产物.

禁止使用 `python -m http.server` 代替端口 `5500` 的静态服务, 因为它默认不提供本项目跨域加载所需的 CORS 响应头.

不得仅凭“已经执行启动命令”声称监听成功. 声明成功前必须实际确认:

- `5500` 端口处于 `LISTEN` 状态;
- 请求 `5500` 服务成功, 且响应包含 `Access-Control-Allow-Origin: *`;
- `6621` 端口处于 `LISTEN` 状态;
- 两个长期运行的终端进程仍然存活;
- 如果酒馆网页已开启“实时监听 → 允许监听”, `pnpm watch` 终端应出现“成功连接到酒馆网页”的日志.

启动时优先使用 Windows 的 `Start-Process` 打开真实 PowerShell 窗口, 分别执行上述两个命令, 并保持窗口在命令结束或用户手动关闭后仍可查看输出. 不要默认使用工具内部的后台 PTY 来承载开发监听服务.

如果受运行环境限制, 只能使用工具内部的后台 PTY, 必须事先或立即明确告诉用户“监听正在后台运行, 没有打开可见 PowerShell 窗口”. 后台 PTY 不等于用户可见终端, 不得声称终端已经显示在 Codex 或桌面上.

如果无法维持长期终端, 必须明确说明“未能保持监听进程”, 不得让用户误以为已经启动. 不得根据 `WORKLOG.md`、`SESSION_HANDOFF.md`、此前对话或历史日志推断当前端口状态, 每次开始工作时都必须重新检查.
